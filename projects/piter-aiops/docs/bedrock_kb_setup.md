...
1. **S3 → Create bucket** (or reuse an existing course bucket)
   ...
   - Versioning: optional
   - Object prefix for this project: `projects/piter-aiops/data/sample_documents/`
...
3. Upload to the bucket (defaults to prefix `projects/piter-aiops/data/sample_documents/`):
   ```bash
...
3. **Step 2 — Choose data source**
   ...
   - **Inclusion prefix:** `projects/piter-aiops/data/sample_documents/` (do not ingest the whole bucket)
