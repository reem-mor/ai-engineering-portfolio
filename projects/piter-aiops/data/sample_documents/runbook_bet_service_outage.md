...
**Applies to:** `bet-service` and `bet-api` in regulated markets (`GIB-UKGC`, `NJ-DGE`, `MGM`)  
...
2. Check recent deployments with deployment correlation tooling.
...
2. Roll back suspect deployment if correlation window matches alert onset (see deployment rollback runbook).
3. Validate database connection pool and active sessions on `postgres`.
4. Check Kafka consumer lag for settlement topics.
...
- **P1 GIB-UKGC:** Page `betting-l2 -> betting-oncall` within 5 minutes.

## Business / regulatory impact

- Direct revenue loss during peak betting windows (demo estimate: $520k/hr for GIB-UKGC outage).
- UKGC and state regulator reporting may be required for prolonged outages.
- Customer trust and SLA credits may apply.

## Dangerous actions warning

Do **not** execute without platform SRE and change-control approval:

- `FLUSHALL` / `FLUSHDB` on Redis session or cache layers.
- Failover or promote Postgres replica without runbook criteria met.
- Disable WAF, rate limits, or auth to "restore traffic quickly".
- Mass `pg_terminate_backend` on all sessions.
- Scale to zero or restart entire fleet without isolating root cause.

Prefer rollback, targeted pool tuning, and dependency validation first.
