...
def _project_incident(inc: dict[str, Any]) -> dict[str, Any]:
    """Project a full incident record to the API detail shape."""
    return {
        "id": str(inc.get("id", "")),
        "title": str(inc.get("title", "")),
        "severity": str(inc.get("severity", "")).upper(),
        "category": str(inc.get("category", "Unknown")),
        "tags": list(inc.get("tags", [])),
        "description": str(inc.get("description", "")),
        "triage_steps": list(inc.get("triage_steps", [])),
        "root_cause": str(inc.get("root_cause", "")),
        "resolution_steps": list(inc.get("resolution_steps", [])),
        "sop_reference": str(inc.get("sop_reference", "")),
        "mttr_minutes": int(inc.get("mttr_minutes", 0)),
        "lessons_learned": str(inc.get("lessons_learned", "")),
        "related_incidents": list(inc.get("related_incidents", [])),
    }


def _project_sop(sop: dict[str, Any]) -> dict[str, Any]:
    """Project a full SOP record to the API detail shape."""
    return {
        "id": str(sop.get("id", "")),
        "title": str(sop.get("title", "")),
        "version": str(sop.get("version", "")),
        "applicability": str(sop.get("applicability", "")),
        "severity_trigger": str(sop.get("severity_trigger", "")).upper() or "N/A",
        "prerequisites": list(sop.get("prerequisites", [])),
        "steps": list(sop.get("steps", [])),
        "escalation_path": str(sop.get("escalation_path", "")),
        "owner": str(sop.get("owner", "Unassigned")),
        "last_updated": str(sop.get("last_updated", "")),
        "related_incidents": list(sop.get("related_incidents", [])),
    }


def _build_stats_payload() -> dict[str, Any]:
    """Build the stats dict shared by /stats and /bootstrap."""
    ...
    try:
        ...
        ingestion_stats: dict[str, Any] = get_retriever().get_ingestion_stats()
        total_vectors: int = int(index_stats.get("total_vectors", 0))
    except RuntimeError:
        total_vectors = 0
        ingestion_stats = {
            "file_documents_indexed": 0,
            "file_chunks_by_extension": {},
        }
    ...
    return {
        ...
        "file_documents_indexed": ingestion_stats.get("file_documents_indexed", 0),
        "file_chunks_by_extension": ingestion_stats.get("file_chunks_by_extension", {}),
...
@router.get(
    "/incidents/recent",
    tags=["knowledge"],
    summary="Most recent incidents (highest INC numbers in the corpus)",
)
async def recent_incidents(limit: int = 4) -> list[dict[str, Any]]:
    """Return a short list of recent incidents for the center-panel quick browse."""
    bounded = max(1, min(limit, 10))
    rows = get_recent_incidents(bounded)
    _logger.info("Recent incidents listed: count=%d", len(rows))
    return rows
...
@router.get(
    "/sops/{sop_id}",
    tags=["knowledge"],
    summary="Full SOP record by id (e.g. SOP-MQ-001)",
)
async def get_sop_detail(sop_id: str) -> dict[str, Any]:
    """Return the full structured SOP used by browse modals."""
    sop = get_sop_by_id(sop_id)
    if sop is None:
        raise HTTPException(status_code=404, detail=f"SOP {sop_id!r} not found")
    return _project_sop(sop)
...
@router.get(
    "/bootstrap",
    tags=["knowledge"],
    summary="Single-call bootstrap payload for the frontend (stats + browse lists)",
)
async def bootstrap() -> dict[str, Any]:
    """Aggregate stats and browse lists in one response to reduce initial page load latency."""
    ...
    payload: dict[str, Any] = {
        ...
        "incidents": incidents,
        "recent_incidents": get_recent_incidents(4),
        "sops": sops,
        "resources": resources,
    }
    _logger.info(
        "Bootstrap: vectors=%d incidents=%d sops=%d resources=%d recent=%d",
        payload["stats"]["total_vectors"],
        payload["stats"]["total_incidents"],
        len(sops),
        len(resources),
        len(payload["recent_incidents"]),
    )
    return payload
