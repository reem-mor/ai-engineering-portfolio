...
3. Build a local PyTorch-backed RAG index with Hugging Face embeddings and FAISS (offline; downloads embedding weights on first run).
4. Build LangChain pipelines for chat memory, document loading, and RAG Q&A.
5. Route support tickets with a LangGraph `StateGraph`.
6. Combine sklearn intent classification with LangChain RAG in one support assistant.
7. Complete **Exercise 01**: breast cancer diagnosis predictor (pure sklearn, interactive CLI).
8. Complete **Exercise 02**: Iris K-means clustering (pure sklearn, scatter plot).
9. Complete **Exercise 03**: spam decision tree (pure sklearn, rule export).
10. Complete **Exercise 04**: spam random forest (pure sklearn, feature importances).
11. Complete **Exercise 05**: Titanic gradient boosting (XGBoost, LightGBM, CatBoost on shared hw03 dataset).
...
- PyTorch RAG: local Hugging Face embeddings (`all-MiniLM-L6-v2`), L2-normalized vectors, FAISS retrieval
...
ML demos (`demos/ml/01`–`03`, `05`–`06`) and exercises 01–05 do **not** require an API key.
...
The PyTorch RAG demo loads **sentence-transformers/all-MiniLM-L6-v2** locally (PyTorch under the hood), embeds `data/risk_analysis_report.txt`, and prints top-k FAISS matches as JSON. First run downloads embedding weights (~90 MB). No OpenAI key required.

```powershell
python demos\ml\06_pytorch_hf_rag.py
python demos\ml\06_pytorch_hf_rag.py path\to\doc.txt "your question here"
```

Compare with `demos\langchain\03_rag_qa.py`, which uses OpenAI embeddings + `RetrievalQA` generation.
...
4. `python demos\ml\06_pytorch_hf_rag.py` prints top-k retrieved chunks for the risk report.
5. `python demos\langchain\02_document_loaders.py` loads `data/risk_analysis_report.txt`.
6. `python exercises\exercise_05_titanic_gradient_boosting.py` prints three booster results.
7. `python -m pytest tests -q` passes.
8. No secrets committed (`git status` should not include `.env`).
...
| `demos/ml/` | scikit-learn demos (01–03), PyTorch MobileNet (05), PyTorch HF RAG (06), sklearn+LangChain combo (04) |
...
| `exercises/hf_rag_model.py` | Hugging Face embedding + FAISS retrieval helpers (used by tests) |
...
| `tests/test_hf_rag_demo.py` | Offline pytest for PyTorch Hugging Face RAG demo |
