"""Invoke a prepared Bedrock Flow alias and print the streamed output."""
...
SAMPLE_TEXT = (
    "Amazon Bedrock Flows let you compose multi-step generative AI applications "
    "using a visual designer or API. Nodes represent steps such as input validation, "
    "prompt templates, knowledge-base retrieval, and conditional routing. "
    "Flows are versioned and invoked through aliases, similar to Lambda versions."
)


def _load_config() -> dict[str, str]:
    load_dotenv(LESSON_ROOT / ".env")
    flow_id = os.getenv("BEDROCK_FLOW_ID", "")
    alias_id = os.getenv("BEDROCK_FLOW_ALIAS_ID", "")
    region = os.getenv("AWS_REGION", "us-east-1")
    profile = os.getenv("AWS_PROFILE")

    if not flow_id or not alias_id:
        logger.error(
            "Set BEDROCK_FLOW_ID and BEDROCK_FLOW_ALIAS_ID in .env "
            "(run create_flow.py first or copy from the Bedrock console)"
        )
        sys.exit(1)
...
def main() -> None:
    config = _load_config()
    custom_text = " ".join(sys.argv[1:]) if len(sys.argv) > 1 else SAMPLE_TEXT
    ...
    print("\n--- Input ---")
    print(custom_text[:500] + ("..." if len(custom_text) > 500 else ""))
    print("\n--- Summary ---")
    print(summary)
